...
def fill_pdf_template(row, pdf_template, output_file):
	fields = [('Name', 'John Smith'),('school', '555-1234')]
	print(row)
	fdf = forge_fdf("",row,[],[],[])

	with open("data2.fdf", "wb") as fdf_file:
	        fdf_file.write(fdf)
	# cmd = "/usr/bin/pdftk '{0}' fill_form '{1}' output '{2}' dont_ask".format(pdf_template, tmp_file, output_file)
	cmd = "/usr/bin/pdftk June4.pdf  fill_form data2.fdf output "+output_file+" flatten"
	print(cmd)
	os.system(cmd)
	#os.remove(tmp_file)
...
if __name__ == "__main__":
	...
	try:
		...
		with open(csv_file, 'r', encoding='mac_roman', newline='') as f:
			# process csv file
			...
			for row in data:
				orderid = row[1][1]
				output_path = os.path.join(output_dir, orderid.replace(" ", "_"))
				output_file = output_path + ".pdf"
				print("test op "+output_path)
